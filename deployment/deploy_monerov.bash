...
cd ~
git clone git@github.com:miltonf/monerov-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd /usr/local/src
sudo git clone git@github.com:monerov/monerov.git
cd monerov
...
sudo -u monerovdaemon /usr/local/src/monerov/build/release/bin/monerov-blockchain-import --input-file $BLOCKCHAIN_DOWNLOAD_DIR/blockchain.raw --batch-size 20000 --database lmdb#fastest --verify off --data-dir /home/monerovdaemon/.monerov
sudo -u monerovdaemon rm -rf $BLOCKCHAIN_DOWNLOAD_DIR
...
bash ~/monerov-pool/deployment/install_lmdb_tools.sh
cd ~/monerov-pool/sql_sync/
